...
@app.get("/")
def home():
    return {"message": "Global DPIA Engine Active"}
...
async def generate_final_report(data: FinalReportRequest):
    try:
        ...
        Transparency: {data.transparency_measures} | Minimization: {data.data_minimization} | Quality: {data.data_quality}
        
        Structure the report as follows:
        ## 2. Executive Summary
        (A professional summary)
        
        ## 3. Data Governance Analysis Table
        You MUST provide the governance analysis in a Markdown table with these columns:
        | Governance Area | Finding | Recommendation | Comment |
        (For the 'Comment' column, leave it empty. Cover: Transparency, Data Minimization, and Data Quality).
        
        ## 4. Formal Risk Assessment Matrix
        You MUST provide the risk matrix in a Markdown table with these columns:
        | Privacy Risk | Recommended Mitigation | Audit Evidence Required | Comment |
        (For the 'Comment' column, leave it empty).
        ...
        doc.add_paragraph("\nReport generated by DPIA Enterprise Systems. Developed for global compliance verification.")

        file_path = f"/tmp/Global_Report.docx"
        doc.save(file_path)
